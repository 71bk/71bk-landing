...
4. 後端完成：
   ...
   - redirect 回前端 `/oauth-callback`
5. 前端進入 callback 頁後呼叫：`GET /api/auth/me`
6. 前端取得 user 資訊與短效 access token（用於後續 `Authorization: Bearer`）
...
| Auth | GET | /api/auth/me | Cookie/Token | 取得登入者資訊 |
...
| search_text | LONGTEXT | 聚合 title+summary+content+tech |
...
## 10. AI 搜尋助手（v1：先做可控且可展示）

### 10.1 資料準備（你的「量化後資料」落地）
- 結構化：techs, category, scores
- 非結構化：content_md / summary
- 聚合欄位：`search_text`

### 10.2 檢索策略（v1 先用 MySQL FULLTEXT）
1. Step A：把 query 拆成可能的 tech/category（規則 or map）
2. Step B：用 tech/category 先縮小候選（結構化）
3. Step C：用 FULLTEXT 對 `search_text` 做相關度排序
...
## 15. 當前專案狀態（Public Portfolio - 71bk-landing）
...
│   └── 使用者故事.md           # 使用者故事
│
├── privacy.html                # 隱私權政策頁面（靜態備份）
└── terms.html                  # 服務條款頁面（靜態備份）
```

### 15.2 技術棧版本（package.json）
...
| **樣式** | tailwindcss | ^4.1.18 | CSS 框架（@tailwindcss/vite 插件） |
| **其他** | postcss / autoprefixer | ^8.5.6 / ^10.4.23 | CSS 處理 |
