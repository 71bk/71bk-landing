...
-- MySQL Schema v3 (i18n Support - Bilingual)
-- Engine: InnoDB | Charset: utf8mb4
-- Updated: 2025-12-26
-- =========================================
-- ⚠️ 警告：此腳本會刪除並重建整個資料庫，所有資料將會遺失！
-- =========================================
-- v3 變更：portfolio_projects 加入雙語支援（_zh / _en）
-- =========================================
...
--    v3: 雙語支援（title, summary, content_md, search_text 改為 _zh / _en）
-- -----------------------------------------
CREATE TABLE IF NOT EXISTS portfolio_projects (
  ...
  -- 雙語內容欄位（v3 新增）
  title_zh         VARCHAR(200)    NOT NULL COMMENT '作品標題（繁體中文）',
  title_en         VARCHAR(200)    NULL COMMENT '作品標題（英文）',
  summary_zh       VARCHAR(500)    NULL COMMENT '作品摘要（繁體中文）',
  summary_en       VARCHAR(500)    NULL COMMENT '作品摘要（英文）',
  content_md_zh    LONGTEXT        NULL COMMENT '作品內容 Markdown（繁體中文，Case Study 詳情）',
  content_md_en    LONGTEXT        NULL COMMENT '作品內容 Markdown（英文，Case Study 詳情）',
  ...
  search_text_zh   LONGTEXT        NULL COMMENT '搜尋用聚合文字-中文（title+summary+content+techs，用於 FULLTEXT）',
  search_text_en   LONGTEXT        NULL COMMENT '搜尋用聚合文字-英文（title+summary+content+techs，用於 FULLTEXT）',
  ...
  FULLTEXT KEY ft_portfolio_projects_search_zh (search_text_zh),
  FULLTEXT KEY ft_portfolio_projects_search_en (search_text_en)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='作品集項目（含 AI 搜尋欄位，雙語支援 v3）';
...
-- -----------------------------------------
-- 9) System Events (admin/system audit)
-- -----------------------------------------
CREATE TABLE IF NOT EXISTS system_events (
  id              BIGINT UNSIGNED NOT NULL AUTO_INCREMENT COMMENT 'Primary key',
  event_type      VARCHAR(100)    NOT NULL COMMENT 'Event type',
  actor           VARCHAR(200)    NULL COMMENT 'Actor identifier',
  status          VARCHAR(50)     NOT NULL COMMENT 'Status',
  payload_json    JSON            NULL COMMENT 'Payload',
  created_at      TIMESTAMP       NOT NULL DEFAULT CURRENT_TIMESTAMP COMMENT 'Created at',
  PRIMARY KEY (id),
  KEY idx_system_events_type_time (event_type, created_at),
  KEY idx_system_events_actor_time (actor, created_at),
  KEY idx_system_events_created_at (created_at)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COMMENT='System event log';
...
-- 9. system_events     - System event log
-- =========================================
